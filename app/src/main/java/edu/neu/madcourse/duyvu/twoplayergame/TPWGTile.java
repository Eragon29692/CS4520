...
public class TPWGTile {
    ...
    private final edu.neu.madcourse.duyvu.twoplayergame.TPWGGameFragment mGame;
    private Owner mOwner = Owner.NEITHER;
    private View mView;
    private TPWGTile mSubTiles[];

    public TPWGTile(TPWGGameFragment game) {
        this.mGame = game;
    }

    public TPWGTile deepCopy() {
        TPWGTile tile = new TPWGTile(mGame);
        tile.setOwner(getOwner());
        if (getSubTiles() != null) {
            TPWGTile newTiles[] = new TPWGTile[9];
            TPWGTile oldTiles[] = getSubTiles();
    ...
    public TPWGTile[] getSubTiles() {
        return mSubTiles;
    }

    public void setSubTiles(TPWGTile[] subTiles) {
    ...
    public Owner findWinner() {
        // Hack since this is single player no need to check for winner
